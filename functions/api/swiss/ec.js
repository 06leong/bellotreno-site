...
function normalizeVehicleStop(rawStop) {
    ...
    return {
        ...
        accessToPreviousVehicle: asBool(rawStop?.accessToPreviousVehicle)
    };
}
...
function normalizeFormation(raw, requestedTrainNumber, requestedDate, evu) {
    ...
    const stops = Array.isArray(raw?.formationsAtScheduledStops)
        ...
        : [];
    const vehicles = extractVehicles(raw).map(normalizeVehicle);
...
export async function onRequestGet(context) {
    ...
    try {
        ...
        if (upstream.status === 404) {
            return json({ available: false, reason: "not_found" }, 200, {
                ...headers,
