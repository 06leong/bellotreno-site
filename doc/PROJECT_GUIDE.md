...
访问统计方面，`BaseLayout.astro` 支持可选 Umami 注入。只有同时配置 `PUBLIC_UMAMI_SCRIPT_URL` 和 `PUBLIC_UMAMI_WEBSITE_ID` 时才会加载，默认限定 `PUBLIC_UMAMI_DOMAINS=real.bellotreno.org`，并启用 `data-do-not-track` 和 `data-exclude-search`。
...
用户浏览器 (前端)
     │
     │  Swiss Formation / Statistics 查询
     ▼
┌──────────────────────────────────┐
│  Cloudflare Pages Functions       │  real.bellotreno.org/api/*
│  - /api/swiss/formation           │
│  - /api/statistics/*              │
│  - 读取 Pages Secrets             │
│  - 不向浏览器暴露 API token        │
...
| **WSGI 服务器** | Gunicorn（1 个 Worker + 4 threads） |
| **容器端口** | 8081 |
...
| **资源限制** | docker compose 中设置 `mem_limit: 384m` |
...
除此之外，Cloudflare Pages Functions 现在负责两个“服务端 token 代理”：
...
这两个 Pages Function 都做 Origin/Referer 校验，响应默认 `Cache-Control: no-store`，避免实时数据被浏览器或边缘节点错误缓存。
...
这不会明显拖慢正常访问：HTML 很小，脚本在同一个部署版本内仍可复用；只有新部署时 URL 变化，浏览器才会下载新脚本。`/_astro/*` 这类带 hash 的资源仍然使用长期 immutable 缓存。
...
| `PUBLIC_UMAMI_DOMAINS` | Plain text | Umami 允许统计的域名，默认 `real.bellotreno.org` |
