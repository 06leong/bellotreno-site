...
4. `INFORMAZIONI SUL TRASPORTO REGIONALE`
5. `INFOLAVORI ABRUZZO`
6. `INFOLAVORI ALTO ADIGE`
7. `INFOLAVORI BASILICATA`
8. `INFOLAVORI CALABRIA`
9. `INFOLAVORI CAMPANIA`
10. `INFOLAVORI EMILIA-ROMAGNA`
11. `INFOLAVORI FRIULI VENEZIA GIULIA`
12. `INFOLAVORI LAZIO`
13. `INFOLAVORI LIGURIA`
14. `INFOLAVORI LOMBARDIA`
15. `INFOLAVORI MARCHE`
16. `INFOLAVORI MOLISE`
17. `INFOLAVORI PIEMONTE`
18. `INFOLAVORI PUGLIA`
19. `INFOLAVORI SARDEGNA`
20. `INFOLAVORI SICILIA`
21. `INFOLAVORI TOSCANA`
22. `INFOLAVORI TRENTINO`
23. `INFOLAVORI UMBRIA`
24. `INFOLAVORI VALLE D'AOSTA`
25. `INFOLAVORI VENETO`

该样例显示 NewsService 已包含运行公告、`INFOTRENI`、区域交通信息以及
按地区组织的 `INFOLAVORI` 条目，并与 `infomobilitaRSS/{true|false}` 存在
...
2. 扫描车站 `partenze`，按配置也可扫描 `arrivi`；默认 `partenze,arrivi` 都会请求，并且每个 station board 类型独立并发执行。
3. 对发现的车次调用 `andamentoTreno`，取得延误、取消、改线、始发/终到和停站信息。
4. 写入 SQLite，并生成前端查询用的 summary、timeseries、trains、stations、relations、ranking 等 JSON cache。统计归属日期使用采集 slot 日期，同时保留真实始发日期 `service_date`；前一晚始发、当天仍在运行或到达的跨午夜列车会计入当天统计。
