...
npm run audit:innerhtml # report remaining innerHTML review locations
...
- `common.ts`: language/theme management, visitor counter, `escapeHtml`, shared
  page behavior.
- `language-events.ts`: typed language-change event dispatch and subscription.
...
- `infomobilita.ts`: provider feed loading, filtering, localization, and the
...
classify that as source corruption by sight alone. Use explicit UTF-8 reads such
as `Get-Content -Encoding UTF8`, then confirm suspicious text with `rg`, `git
diff`, or byte/code-point inspection. `.editorconfig` declares UTF-8 and LF for
editors that support it. If the file itself contains mojibake, fix the source
text directly instead of documenting around it.

## Dependency Maintenance

- `.github/dependabot.yml` monitors the root npm lockfile and GitHub Actions on
  a weekly schedule.
- npm patch and minor releases are grouped to keep review noise low. Major npm
  releases remain separate PRs and require migration review.
- GitHub Actions updates are grouped together and must still pass the full CI
  baseline.
- Do not enable Dependabot for the Python services until their currently
  unbounded `requirements.txt` files have an intentional constraint/lock
  policy.
- Do not enable automatic Docker base-image updates until the supported Python
  runtime policy for both VPS services is documented and tested.
- Never auto-merge dependency PRs solely because they are generated by a bot.
  Run `npm ci`, `npm run check`, and `npm run build`, and read release notes for
  major or security-sensitive updates.
