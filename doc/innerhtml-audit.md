...
- `src/client/infomobilita.ts`: RFI cards and Trenitalia infomobility cards use
  DOM builders. Trenitalia NewsService descriptions are parsed through a small
  allowlist sanitizer before rendering, so upstream HTML is not inserted raw
  into the document. The live Trenitalia UI no longer renders separate
  `infomobilitaRSS/{true|false}` subviews.
